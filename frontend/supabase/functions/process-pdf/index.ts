...
async function extractTextFromPDF(base64: string): Promise<string> {
  try {
    ...
    if (err.message === "INSUFFICIENT_TEXT") {
      throw new Error(
        "This PDF appears to be scanned or image-based. Please copy and paste your text using Paste Notes instead.",
      );
    }
    try {
      const binaryStr = atob(base64);
...
function smartChunk(text: string): string[] {
  ...
  const isHeading = (line: string): boolean => {
    ...
    if (trimmed.length < 80) {
      if (trimmed === trimmed.toUpperCase() && /[A-Z]/.test(trimmed))
        return true;
      if (
        /^(\d+[\.\)]|chapter|section|topic|unit|part|introduction|conclusion|summary)/i.test(
          trimmed,
        )
      )
        return true;
  ...
  if (chunks.length <= 1) {
    ...
    const wordsPerChunk = Math.ceil(
      words.length / Math.min(6, Math.ceil(words.length / 500)),
    );
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a teacher. Output ONLY raw valid JSON. No markdown. No backticks. No explanation. Just the JSON.",
            },
...
{"title":"short title under 8 words","explanation":"explain in 3 simple sentences","analogy":"Nigerian real-world comparison in 2 sentences","steps":"Step 1: point. Step 2: point. Step 3: point.","question":"one reflective question"}`;
    ...
    if (lesson && lesson.title) {
      const stepsText =
        lesson.steps || "Step 1: Read. Step 2: Understand. Step 3: Review.";
      const stepParts = stepsText
        .split(/step \d+:/i)
      ...
      lessons.push({
        ...
        level_2:
          lesson.analogy ||
          "Think of this like a journey through the material.",
        level_3: stepsText,
        level_4:
          lesson.question ||
          "Think about this: What is the most important idea?",
        level_3_visual: {
          type: "steps",
          title: "Key Points",
          items: [
            {
              label: "Step 1",
              text: stepParts[0]?.trim().substring(0, 60) || "First key idea",
            },
            {
              label: "Step 2",
              text: stepParts[1]?.trim().substring(0, 60) || "Second key idea",
            },
            {
              label: "Step 3",
              text: stepParts[2]?.trim().substring(0, 60) || "Third key idea",
            },
          ],
        },
      });
      ...
      console.log(
        `Chunk ${i} - JSON parse failed, raw: ${content.substring(0, 200)}`,
      );
    }
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { pdfBase64, pasteText, topicTitle, studentId, classId, gradeLevel } =
      body;
    ...
    if (!pdfBase64 && !pasteText) {
      return new Response(
        JSON.stringify({
          error:
            "No content provided. Please upload a PDF or paste your notes.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    if (!studentId && !classId) {
      return new Response(
        JSON.stringify({
          error: "Session error. Please log out and log back in.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    const lessons = await generateLessonsFromChunks(
      chunks,
      title,
    ...
    if (!lessons || lessons.length === 0) {
      return new Response(
        JSON.stringify({
          error: "Could not generate lessons. Please try again.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 500,
